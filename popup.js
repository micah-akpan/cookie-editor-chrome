...
const overwriteCookieCheckBox = document.getElementById(
  "cb-cookie-overwrite-old"
);
...
document.forms.item(0).addEventListener("submit", async (e) => {
  ...
  try {
    ...
    const allowListOfCookiePropsObj =
      getAllowListOfCookieProps(parsedCurrentCookie);
...
async function getCookie(cookieHostURL, cookieName) {
  ...
  if (cookie === null) {
    throw new Error(
      `There's probably no cookie with the name in the specified domain: ${cookieName}`
    );
  }
...
function setUpdatedCookieValue(newCookie) {
  const newCookieValueElem = newCookieValue.insertAdjacentElement(
    "afterend",
    getNewCookieElem()
  );
...
async function getNewCookieValue() {
  ...
  if (cookieHostURLValue.cookieHostURL) {
    ...
    const cookie = await getCookie(
      cookieHostURLElem.value.trim(),
      cookieKey.value
    );
    cookieToBeChanged = cookie.value;
  }

  const mutatedCookie = mutate(
    cookieToBeChanged,
    cookieIndex.value,
